def plot_training_log(training_log, folder="", epoch = 0):
    ...
    plt.figure(figsize=(6, 6))
    plt.plot(training_log["average_reward"], label="reward")
    for key, value in training_log["curiosities"].items():
        plt.plot(value, label=f"curiosity {key}")
    plt.plot(training_log["total_reward"], label="total")
    ...
    plt.figure(figsize=(6, 6))
    plt.plot(training_log["Q_for_actor"], label="Q_for_actor")
    plt.plot(training_log["entropy_for_actor"], label="entropy_for_actor")
    plt.plot(training_log["total_imitation_loss"], label="total_imitation_loss")
    plt.plot(training_log["actor_loss"], label="actor_loss")
    ...
    plt.figure(figsize=(6, 6))
    plt.plot(training_log["Q_target"], label="Q Target")
    for i, critic_loss in enumerate(training_log["critic_predictions"]):
        plt.plot(critic_loss, label=f"Critic Predictions")
    ...
    for key, alpha in training_log["alphas"].items():
        plt.plot(alpha, label=f"Alpha {key}")
